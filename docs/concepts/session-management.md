...
|---------|-----------------|-----|
| **PR Code Reviews** | Persistent per PR + model | A session is maintained for each (PR, model) pair. Re-reviews reuse the same session, allowing the model to build on prior analysis. |
...
- **Reviews**: Session reuse means re-reviewing the same PR with the same model builds on the model's memory of prior findings. This is powerful for iterative refinement.
